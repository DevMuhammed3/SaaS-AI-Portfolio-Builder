      ...
      {steps.map((step, index) => (
        <div key={step.number} className="flex items-center">
          ...
          {index < steps.length - 1 && (
            <div
              className={`w-24 h-1 mx-2 ${currentStep > index + 1 ? "bg-primary" : "bg-gray-200"
                }`}
            />
          )}
        </div>
  ...
  return (
    ...
    >
      <div className="flex items-center">
        <div
          className={`w-6 h-6 rounded-full border flex items-center justify-center mr-3 ${isSelected ? "border-primary" : "border-gray-400"
            }`}
        >
      ...
      window.paypal
        .Buttons({
          ...
          onApprove: async (_, actions) => {
            ...
            // 👉 Optionally send order info to your backend for verification
            const token = await getToken(); // Clerk JWT

            await apiClient.post(
              "/api/user/paypal/verify-payment",
              { orderId: order.id },
              {
                headers: {
